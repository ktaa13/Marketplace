...
import com.mycompany.jobsapi.security.WebSecurityConfig;
...
public class KeycloakInitializerRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        clientRepresentation.setDefaultRoles(new String[]{MARKETPLACE_APP_ROLES.get(0)});
        ...
        List<UserRepresentation> userRepresentations = MOVIES_APP_USERS.stream().map(userPass -> {
            ...
            if ("admin".equals(userPass.getUsername())) {
                clientRoles.put(MOVIES_APP_CLIENT_ID, MARKETPLACE_APP_ROLES);
            } else {
                clientRoles.put(MOVIES_APP_CLIENT_ID, Collections.singletonList(MARKETPLACE_APP_ROLES.get(0)));
            }
    ...
    private static final List<String> MARKETPLACE_APP_ROLES = Arrays.asList(WebSecurityConfig.JOBS_CUSTOMER,
            WebSecurityConfig.JOBS_STAFF);
